...
class Guest extends Model\Model {
    ...
    public function signup(string $username, string $email, string $password) {
        if (!empty(Database::query("SELECT * FROM app_accounts WHERE username LIKE ?",[$username])[0]))
            throw new Exception(_("This username is already taken."), 1100);
                
        if (!empty(Database::query("SELECT * FROM app_accounts WHERE email LIKE ?", [$email])[0]))
            throw new Exception(_("This email address is already taken."), 1101);
        ...
        if (App::get("APP_WELCOME")) {
            ...
            Mailer::send(sprintf(_("New Account | %s"), App::get("APP_NAME")), App::get("MAIL_RECEIVER"), Template::get("/_emails/adminWelcome.tpl", [
                "var" => (object) [
                    "username"  => $account->get("username")
                ],
                "app" => (object) [
