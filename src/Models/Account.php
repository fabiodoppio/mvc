        ...
        if (isset($this->data[0][$name])) {
            if ($name == "username")
                if (!empty(Database::query("SELECT * FROM ".$this->table." WHERE username LIKE ?",[$value])[0]))
                    throw new Exception(_("This username is already taken."), 1087);

            if ($name == "email")
                if (!empty(Database::query("SELECT * FROM ".$this->table." WHERE email LIKE ?", [$value])[0]))
                    throw new Exception(_("This email address is already taken."), 1088);
    ...
    public function attempt_login(string $password) {
        if (count(Database::query("SELECT * FROM ".$this->table."_log WHERE id = ? AND event = ? AND timestamp BETWEEN (NOW() - INTERVAL ? MINUTE) AND NOW()", 
            [$this->get("id"), "failed_login_attempt", 20])) > 2) 
                throw new Exception(_("You have entered the password incorrectly too many times. Please wait 20 minutes and try again."), 1089);

        if (!password_verify($password, $this->get("password"))) {
            $this->log("failed_login_attempt");
            throw new Exception(_("You have entered an incorrect password."), 1090);
        }

        if ($this->get("role") < self::GUEST)
            throw new Exception(_("Your account is suspended or deactivated."), 1091);
    }
    ...
    public function attempt_2fa_login(string $code) {
        if (count(Database::query("SELECT * FROM ".$this->table."_log WHERE id = ? AND event = ? AND timestamp BETWEEN (NOW() - INTERVAL ? MINUTE) AND NOW()", 
            [$this->get("id"), "failed_2fa_login_attempt", 20])) > 2) 
                throw new Exception(_("You have entered an invalid PIN code too many times. Please wait 20 minutes and try again."), 1092);

        if ($this->get("code") != str_replace(' ', '', $code) || $this->get("timestamp") < strtotime('-15 minutes')) {
            $this->log("failed_2fa_login_attempt");
            throw new Exception(_("This PIN code is invalid."), 1093);
        }
    ...
    public function request_recovery_code() {
        if (count(Database::query("SELECT * FROM ".$this->table."_log WHERE id = ? AND event = ? AND timestamp BETWEEN (NOW() - INTERVAL ? MINUTE) AND NOW()", 
            [$this->get("id"), "recovery_code_requested", 60])) > 2) 
                throw new Exception(_("You have tried to request a confirmation code too many times. Please wait 60 minutes and try again."), 1094);
    ...
    public function recover(string $code, string $password) {
        if (count(Database::query("SELECT * FROM ".$this->table."_log WHERE id = ? AND event = ? AND timestamp BETWEEN (NOW() - INTERVAL ? MINUTE) AND NOW()", 
            [$this->get("id"), "failed_recovery", 60])) > 2) 
                throw new Exception(_("You have entered an invalid confirmation code too many times. Please wait 60 minutes and try again."), 1095);

        if ($this->get("code") != str_replace(' ', '', $code) || $this->get("timestamp") < strtotime('-15 minutes')) {
            $this->log("failed_recovery");
            throw new Exception(_("This confirmation code is invalid."), 1096);
        }

        if ($this->get("role") < self::DEACTIVATED)
            throw new Exception(_("Your account cannot be restored."), 1097);
    ...
    public function verify(string $code) {
        if (count(Database::query("SELECT * FROM ".$this->table."_log WHERE id = ? AND event = ? AND timestamp BETWEEN (NOW() - INTERVAL ? MINUTE) AND NOW()", 
            [$this->get("id"), "failed_verification", 60])) > 2) 
                throw new Exception(_("You have entered an invalid confirmation code too many times. Please wait 60 minutes and try again."), 1098);
    
        if ($this->get("code") != str_replace(' ', '', $code) || $this->get("timestamp") < strtotime('-15 minutes')) {
            $this->log("failed_verification");
            throw new Exception(_("This confirmation code is invalid."), 1099);
        }
