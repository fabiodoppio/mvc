                            ...
                            {% foreach ($accounts as $item): %}
                                <li class="list-item" data-id="{{$item->id}}">
                                    <div class="list-item-header">
                                        <span class="avatar">{% if ($item->meta->avatar??""): %}<img src="{{$app->url}}{{$directory->uploads}}/{{$item->avatar}}"/>{% endif; %}</span><span class="username">{{$item->username}}</span><span class="id">id: {{$item->id}}</span>
                                    </div>
                                    <div class="list-item-content">
                                        <h3 class="title">{{"Edit Account"}}</h3>
                                        <div class="avatar">
                                            {% if ($item->meta->avatar??""): %}
                                                <img src="{{$app->url}}{{$directory->uploads}}/{{$item->meta->avatar}}"/>
                                            {% endif; %}
                                        </div>
                                        ...
                                        <form data-request="admin/account/edit">
                                            ...
                                            <label for="displayname-{{$item->id}}">
                                                ...
                                                <select id="displayname-{{$item->id}}" name="displayname" required>
                                                    {% $selected = ($item->meta->displayname == $item->username) ? "selected" : ""; %}
                                                    <option value="{{$item->username}}" {{$selected}}>{{$item->username}}</option>
                                                    {% if ($item->meta->company): %}
                                                        {% $selected = ($item->meta->displayname == $item->company) ? "selected" : ""; %}
                                                        <option value="{{$item->company}}" {{$selected}}>{{$item->company}}</option>
                                                    {% endif; %}
                                                    {% if ($item->meta->firstname): %}
                                                        {% $selected = ($item->meta->displayname == $item->firstname) ? "selected" : ""; %}
                                                        <option value="{{$item->firstname}}" {{$selected}}>{{$item->firstname}}</option>
                                                    {% endif; %}
                                                    {% if ($item->meta->lastname): %}
                                                        {% $selected = ($item->meta->displayname == $item->meta->lastname) ? "selected" : ""; %}
                                                        <option value="{{$item->lastname}}" {{$selected}}>{{$item->lastname}}</option>
                                                    {% endif; %}
                                                    {% if ($item->firstname && $item->lastname): %}
                                                        {% $selected = ($item->meta->displayname == $item->firstname." ".$item->lastname) ? "selected" : ""; %}
                                                        <option value="{{$item->firstname}} {{$item->lastname}}" {{$selected}}>{{$item->firstname}} {{$item->lastname}}</option>
                                                        {% $selected = ($item->meta->displayname == $item->lastname." ".$item->firstname) ? "selected" : ""; %}
                                            ...
                                            {% foreach ($item->meta as $name => $value): %}
                                                {% if ($name != "displayname" && $name != "avatar" && $name != "language" && $name != "newsletter"): %}
