...
class AccountController extends Controller {
    ...
    public function beforeAction() {
        ...
        if (time() < strtotime($this->account->get("lastaction")) + 2)
            throw new Exception(_("Too many requests in a short time."), 1033);

        $this->account->set("lastaction", date("Y-m-d H:i:s", time()));

        if ($this->account->get("role") < Model\Account::GUEST)
            throw new Exception(_("Your account is suspended or deactivated."), 1034);
    }
        ...
        switch($request) {
            case "/account/login":
                ...
                if (!App::get("APP_LOGIN"))
                    throw new Exception(_("Log in not possible at the moment."), 1035);
                ...
                if (empty($_POST["credential"]) || empty($_POST["pw"]))
                    throw new Exception(_("Required input not found."), 1036);
                ...
                if ($account->get("2fa"))
                    if (empty($_POST["code"])) {
                        ...
                        Ajax::add('form[data-request="account/login"', Template::get("/_forms/2fa.tpl"), Ajax::REPLACE);
                ...
                break;
            default: 
                throw new Exception(sprintf(_("Action %s not found."), $request), 1037);
        }
    }
        ...
        switch($request) {
            case "/account/signup":
                ...
                if (!App::get("APP_SIGNUP") || App::get("APP_MAINTENANCE") || !empty($_POST["firstname"]))
                    throw new Exception(_("Sign up not possible at the moment."), 1038);

                if ($this->account->get("role") != Model\Account::GUEST)
                    throw new Exception(_("Your account does not have the required role."), 1039);
        ...
        switch($request) {
            case "/account/recovery":
                ...
                if (empty($_POST["credential"]))
                    throw new Exception(_("Required input not found."), 1042);
                ...
                if (empty($_POST["code"])) {
                    ...
                    Ajax::add('form[data-request="account/recovery"]', Template::get("/_forms/recovery.tpl", ["credential" => $credential]), Ajax::REPLACE);
                }
                else {
                    if (empty($_POST["pw1"]) || empty($_POST["pw2"]))
                        throw new Exception(_("Required input not found."), 1043);
    ...
    public function personalAction(string $request) {
        if (App::get("APP_MAINTENANCE") && $this->account->get("role") != Model\Account::ADMINISTRATOR)
            throw new Exception(_("App currently offline. Please try again later."), 1045);

        if ($this->account->get("role") < Model\Account::USER)
            throw new Exception(_("Your account does not have the required role."), 1046);
        ...
        switch($request) {
            ...
            case "/account/personal/avatar/upload":
                ...
                if ($this->account->get("role") < Model\Account::VERIFIED)
                    throw new Exception(_("You have to verify your email address before you can upload an avatar."), 1048);
            ...
            case "/account/personal/avatar/delete":
                ...
                break;
            default: 
                throw new Exception(sprintf(_("Action %s not found."), $request), 1049);
        }
    }
    ...
    public function securityAction(string $request) {
        if (App::get("APP_MAINTENANCE") && $this->account->get("role") != Model\Account::ADMINISTRATOR)
            throw new Exception(_("App currently offline. Please try again later."), 1050);
        ...
        switch($request) {
            case "/account/security/edit":
                ...
                if (!empty($_POST["pw"]) && !empty($_POST["pw1"]) && !empty($_POST["pw2"])) {
                    if (!password_verify($_POST["pw"], $this->account->get("password"))) 
                        throw new Exception(_("Your current password does not match."), 1052);
            ...
            case "/account/security/2fa":
                ...
                if ($this->account->get("role") < Model\Account::VERIFIED)
                    throw new Exception(_("You have to verify your email address before you can acitvate the 2-Factor Authentication."), 1053);
            ...
            case "/account/security/deactivate":
                ...
                if ($this->account->get("role") == Model\Account::ADMINISTRATOR)
                    throw new Exception(_("You can not deactivate your account."), 1054);
                ...
                break;
            default: 
                throw new Exception(sprintf(_("Action %s not found."), $request), 1055);
        }
    }
    ...
    public function emailAction(string $request) {
        if (App::get("APP_MAINTENANCE") && $this->account->get("role") != Model\Account::ADMINISTRATOR)
            throw new Exception(_("App currently offline. Please try again later."), 1056);

        if ($this->account->get("role") < Model\Account::USER)
            throw new Exception(_("Your account does not have the required role."), 1057);
        ...
        switch($request) {
            ...
            case "/account/email/verify":
                ...
                if (empty($_POST["code"])) {
                    ...
                    Ajax::add('form[data-request="account/email/verify"]', Template::get("/_forms/verify.tpl"), Ajax::REPLACE);
                }
                else {
                ...
                break;
            default: 
                throw new Exception(sprintf(_("Action %s not found."), $request), 1059);
        }
    }
        ...
        switch($request) {
            ...
            case "/account/feedback":
                ...
                if (empty($_POST["displayname"]) || empty($_POST["email"]) || empty($_POST["message"]) || !empty($_POST["firstname"]))
                    throw new Exception(_("Required input not found."), 1062);

                if (isset($_FILES["attachment"]) && $this->account->get("role") < Model\Account::VERIFIED)
                    throw new Exception(_("You have to verify your email address before you can send a message with an attachment."), 1063);

                App::set_locale_runtime(App::get("APP_LANGUAGE"));
                Mailer::send(sprintf(_("New Message | %s"), App::get("APP_NAME")), App::get("MAIL_RECEIVER"), Template::get("/_emails/adminMessage.tpl", [
                    "var" => (object) [
                ...
                break;
            default: 
                throw new Exception(sprintf(_("Action %s not found."), $request), 1064);
        }   
    }
