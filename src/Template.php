...
class Template {
    ...
    private static function cache_file(string $file) { 
        ...
        if (App::get("APP_DEBUG") || !file_exists($cache) || filemtime($cache) < filemtime(App::get("DIR_ROOT").App::get("DIR_VIEWS").'/'.App::get("APP_LANGUAGE").'/'.$file)) {
    ...
    private static function include_files(string $file) {
        if (!file_exists(App::get("DIR_ROOT").App::get("DIR_VIEWS").'/'.App::get("APP_LANGUAGE").'/'.$file))
            throw new Exception(sprintf(_("Template %s not found."), $file));

        $template = file_get_contents(App::get("DIR_ROOT").App::get("DIR_VIEWS").'/'.App::get("APP_LANGUAGE").'/'.$file);
