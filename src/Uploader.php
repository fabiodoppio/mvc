     ...
     */
     public static function upload(array $file, int $type) {
        ...
        switch($type) {
            case self::ATTACHMENT:
                if (!in_array($filetype, ["image/jpeg", "image/jpg", "image/png", "application/pdf", "text/plain"]))
                    throw new Exception(_("File has to be a jpeg, jpg, png, pdf or txt."), 1008);

                if ($file["size"] > App::get("RULE_ATT_FILESIZE"))
                    throw new Exception(sprintf(_("File exceeds the maximum allowed file size of %s KB."), App::get("RULE_ATT_FILESIZE") / 1000), 1009);
            ...
            case self::AVATAR: 
                if (!in_array($filetype, ["image/jpeg", "image/jpg", "image/png"]))
                    throw new Exception(_("File has to be a jpeg, jpg or png."), 1010);

                if ($file["size"] > App::get("RULE_AVA_FILESIZE"))
                    throw new Exception(sprintf(_("File exceeds the maximum allowed file size of %s KB."), App::get("RULE_AVA_FILESIZE") / 1000), 1011);

                if ($imagesize[0] != $imagesize[1])
                    throw new Exception(_("Avatar has to be squared."), 1012);
        ...
        if (!move_uploaded_file($file["tmp_name"], App::get("DIR_ROOT").$dirname."/".$basename.".".$extension))
            throw new Exception(_("File could not be uploaded."), 1013);
